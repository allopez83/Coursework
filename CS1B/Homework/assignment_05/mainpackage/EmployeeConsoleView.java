...
public class EmployeeConsoleView {
	...
	/**
	 * Searches for an employee using their SSN
	 */
	public void contains(java.util.LinkedList<Employee> theList){
		boolean found = false;
		int counter = 0;
		int limit = theList.size();
		String target = "";
		
		String targetMessage = "What is the employee's Social Security Number?", targetError = "Error: enter an appropriate SSN, numbers only!";
		target = UserInputSSN(targetMessage, targetError);
		Employee searchEmpl = new Employee("",target,0.00);
		
		for(counter = 0; (counter<limit) && (!found); counter++){
			if (A05_Main.TESTING) System.out.println("Loop#" + counter);
			if( (theList.get(counter)).contains(searchEmpl) )
				found = true;
		}
		if (A05_Main.TESTING) System.out.println("It is here: " + found);
		if (found)
			System.out.println("Requested entry found:\n" +
					theList.get(counter-1));
		else{
			System.out.println("Requested entry not found!");
		}
		kb.close();
	}
