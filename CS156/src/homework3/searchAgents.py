...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        foodGrid = self.corners
        self.startState = self.startingPosition, foodGrid
        ...
        for action in [Directions.NORTH, Directions.SOUTH,
            ...
            if not self.walls[nextx][nexty]:
                if (nextx, nexty in self.corners):
                    food_locations = set(foodGrid)
                    food_locations.discard((nextx, nexty))
                    nextState = ((nextx, nexty), tuple(food_locations))
                else:
                    nextState = ((nextx, nexty), foodGrid)
                cost = 1
                successors.append( ( nextState, action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    if foodGrid == ():
        return 0
    # Find distance for all food
    for food in foodGrid:
        foodDistance.append((linearDistance(position, food), food))
    foodDistance.sort()
    # Distance to closest food
    closest = foodDistance[0][1]
    heuristic = linearDistance(position, closest)
...
def foodHeuristic(state, problem):
    ...
    foodDistance.sort()
    # Distance to closest food
    heuristic, closest = foodDistance[0]
    # print position, "->", closest, "h:", heuristic
