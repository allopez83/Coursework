...
public class SubsetSumC
{
   public static void main(String[] args) throws Exception
   {
      ...
      ArrayList<Sublist<iTunesEntry>> collection = new ArrayList<Sublist<iTunesEntry>>();
      Sublist<iTunesEntry> list = new Sublist<iTunesEntry>(masterList), perfectSublist = null;
      ...
      collection.add(new Sublist<iTunesEntry>(masterList)); // this represents
                                                            // the 0 set
         ...
         for (int k = 0; k < collectionSize && !perfectFound; k++)
         {
            ...
            if (sum <= limit)
            {
               try
               {
                  if (sum == limit)
                  {
                     perfectSublist = list.addItem(i, songLength);
                     perfectFound = true;
                  }
                  if (sum < limit)
                  {
   ...
   static class Sublist<iTunesEntry> implements Cloneable
   {
       ...
       * @param songLength length of time the iTunes entry lasts
...
Algorithm Elapsed Time: 0.088 seconds.
