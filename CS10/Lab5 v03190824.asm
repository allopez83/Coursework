...
# Matrix multiply
#jal   matrixMult
addiu  $t6, $t6, 0 # Offsets are zero
addiu  $t7, $t7, 0
move  $t8, $s5 # 'top left' matrices, so exactly at start of both matrices
move  $t9, $s6
jal   blockMult
# Print results
...
blockMult:    # Block Matrix Multiplication Code
	...
	move  $s0, $t6         # 1st loop layer - Y axis of matrix 1 with predefined offsets
L11:	move  $s1, $t7         # 2nd loop layer - X axis of matrix 2 with predefined offsets
...
L13:    mul   $t0, $s2, $s4    # Find the appropriate row by multiplying offset by matrix size
	...
	addiu $s1, $s1, 1      # Increment and branch as necessary
	bne   $s1, $t4, L12
	addiu $t3, $t3, 4      # Increment counter $t3 to next matrix row
	addiu $s0, $s0, 1      # Increment and branch as necessary
	bne   $s0, $t4, L11
